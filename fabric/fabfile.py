...
def deploy_and_run(jar, class_name, classpath):
    ...
    with cd('/tmp'):
        ...
        hadoop_classpath = ':'.join(['/tmp/libs/' + os.path.basename(path) for path in classpath.split(';')])
...
def put_if_absent(local_paths, remote_dir):
    ...
    remote_dir    
    """
    local_artifacts = {os.path.basename(local_path) : local_path for local_path in local_paths}
